...
{
    float error_old = 0f;
    //The controller will be more robust if you are using a further back samples
    ...
    {
        float output = CalculatePIDOutput(this.gain_P, this.gain_I, this.gain_D,error);
        return output;
    }
    ...
    public float GetFactorFromPIDController(float gain_P, float gain_I, float gain_D, float error)
    {
        ...
        this.gain_I = gain_I;
        this.gain_D = gain_D;
        float output = CalculatePIDOutput(gain_P, gain_I, gain_D,error);
        return output;
    }
    ...
    private float CalculatePIDOutput(float gain_P, float gain_I, float gain_D, float error)
    {
        //Test to use the average error as input
        float averageAmount = error_sum2;
